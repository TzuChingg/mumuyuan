            ...
            <table v-if="!isLoading" class=" table table-hover align-middle table-sm reduce-spacing">
              ...
              <tbody class="fs-4 text-nowrap" v-for="(user, index) in filteredUsersList" :key="index">
                <tr>
                  <td>{{ user.name }}</td>
                  <td>手機:{{ user.phone }}</td>
                  <td class="text-end "><button class="btn btn-outline-primary me-5" data-bs-toggle="modal"
                      :data-bs-target="'#goBad' + index">黑名單</button></td>
                </tr>
                <!-- model -->
            ...
            <table v-if="!isLoading" class=" table table-hover align-middle table-sm reduce-spacing">
              ...
              <tbody class="fs-4 text-nowrap" v-for="(user, index) in filteredBadUsersList" :key="index">
                <tr>
                  <td>{{ user.name }}</td>
                  <td>手機:{{ user.phone }}</td>
                  <td class="text-end "><button class="btn btn-primary me-5" data-bs-toggle="modal"
                      :data-bs-target="'#goGood' + index">原諒你</button></td>
                </tr>
                <!-- model -->
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content border-0">
                      ...
                      <div class="modal-footer border-0">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal"
                          @click="goGood(user.id)">確認</button>
                      </div>
                    </div>
                  </div>
                </div>
